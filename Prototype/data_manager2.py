...
class DataManger:
    ...
    def __init__(self, data_path: Path, user_embedding_path: Path = None, item_embeddings_path: Path = None, user_key: str = 'user_id', item_key: str = 'app_id'):
        (self.URM_train, self.URM_test, 
         self.user_embeddings, self.item_embeddings) = self.load_data(user_embedding_path, data_path, item_embeddings_path, user_key=user_key, item_key=item_key)
    ...
    def load_data(self, user_embeddings_path: Path, data_path: Path, item_embeddings_path: Path = None, user_key: str = 'user_id', item_key: str = 'app_id'):
        ...
        # MODIFICA 2: Tutta la logica degli user embedding è ora condizionale
        user_embeddings = None
        if user_embeddings_path:
            x_user = np.load(user_embeddings_path, allow_pickle=True)
            print(f"files {x_user.files}")
            all_user_vectors = x_user['embeddings']
            embedding_user_ids = np.array([int(i) for i in x_user[user_key]])
        ...
        if item_embeddings_path:
            ...
            print(f"files {x_item.files}")
            all_item_vectors = x_item['embeddings']
            embedding_item_ids = np.array([int(i) for i in x_item[item_key]])
