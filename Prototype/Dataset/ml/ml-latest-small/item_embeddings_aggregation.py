...
MODEL = 'e5'  # Modello di embedding da utilizzare
def calculate_user_embeddings():
    ...
    with np.load(f'item_embeddings_{MODEL}.npz') as data:
    ...
    interactions_df = pd.read_csv('train_recommendations.csv', dtype={'user_id': np.uint32, 'app_id': np.uint32})
    ...
    np.savez_compressed(
        f'mean_user_embeddings_{MODEL}.npz',
        user_id=user_ids,
