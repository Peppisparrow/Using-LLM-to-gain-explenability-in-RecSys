...
DATA_PATH = Path('Prototype/Dataset/ml_small/tuning')

MODEL = 'T5Gemma'  # Modello di embedding da utilizzare
def calculate_user_embeddings():
    ...
    with np.load(DATA_PATH / f'item_embeddings_{MODEL}.npz') as data:
    ...
    interactions_df = pd.read_csv(DATA_PATH / 'train_recommendations.csv', dtype={'user_id': np.uint32, 'app_id': np.uint32})
    ...
    np.savez_compressed(
        DATA_PATH / f'mean_user_embeddings_{MODEL}.npz',
        user_id=user_ids,
