...
BASE_RECOMMENDATIONS_PATH = "Dataset/ml/ml-latest-small/tuning/gemma/model-no-4bit/recommendations.json"
FINETUNED_RECOMMENDATIONS_PATH = "Dataset/ml/ml-latest-small/tuning/gemma/model-no-4bit/gemma3-grpo/recommendations.json"
...
def generate_embeddings(model_identifier: str, input_json_path: str, output_dir: str, model_name_for_files: str):
    ...
    output_npz_last = os.path.join(output_dir, f"UserRecommendations_GemmaLast.npz")
    output_npz_mean = os.path.join(output_dir, f"UserRecommendations_GemmaMean.npz")
    checkpoint_path_last = os.path.join(output_dir, f"checkpoint_last.pkl")
    checkpoint_path_mean = os.path.join(output_dir, f"checkpoint_mean.pkl")
    ...
    if not items_to_process:
        ...
        for i, (user_id, recommendation_text) in enumerate(tqdm(items_to_process, desc=f"Processing ({model_name_for_files})")):
    ...
    convert_to_npz(final_last_results, output_npz_last, id_key='user_id')
    convert_to_npz(final_mean_results, output_npz_mean, id_key='user_id')
