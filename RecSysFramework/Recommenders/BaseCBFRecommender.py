...
from RecSysFramework.Recommenders.BaseRecommender import BaseRecommender as _BaseRecommender
...
class BaseUserItemCBFRecommender(_BaseRecommender):
    """
    This class refers to a BaseRecommender which uses content features, it provides only one function
    to check if users and items exist that have no features
    """

    def __init__(self, URM_train, UCM_train, ICM_train, verbose = True):
        super(BaseUserItemCBFRecommender, self).__init__(URM_train, verbose = verbose)
