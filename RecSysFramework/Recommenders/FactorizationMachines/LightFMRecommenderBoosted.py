...
from RecSysFramework.Recommenders.BaseCBFRecommender import BaseItemCBFRecommender, BaseUserCBFRecommender, BaseUserItemCBFRecommender
...
class _BaseLightFMWrapper(BaseRecommender, Incremental_Training_Early_Stopping):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute = None):
        ...
        if not self.vectorized_mode:
            ...
            # Repeat each user ID n_items times
            # Ex: [u1, u2] -> [u1, u1, u1, u2, u2, u2] for 3 items
            users_for_predict = np.repeat(user_id_array, n_items)

            # Repeat the list of items n_users times
            # Ex: [i1, i2, i3] -> [i1, i2, i3, i1, i2, i3] for 2 users
            items_for_predict = np.tile(items_to_compute, n_users)

            # 2. Make a single prediction call for all pairs
            all_scores = self.lightFM_model.predict(users_for_predict,
                                                    items_for_predict,
                                                    item_features=self.ICM_train,
                                                    user_features=self.UCM_train,
                                                    num_threads=self.num_threads) # Use multiple cores

            # 3. Reshape the flat array of scores back into a (n_users, n_items) matrix
            item_scores = all_scores.reshape(n_users, n_items)

            # If you are not scoring all items, you need a full-sized matrix
            if items_to_compute.shape[0] != self.n_items:
                full_item_scores = -np.ones((n_users, self.n_items)) * np.inf
                # Place the computed scores in the correct columns
                full_item_scores[:, items_to_compute] = item_scores
                return full_item_scores

            return item_scores



    def _init_model(self, loss, sgd_mode, n_components, item_alpha, user_alpha, learning_rate, vectorized_mode = False, num_threads = 4):
        ...
        self.vectorized_mode = vectorized_mode
...
class LightFMUserItemHybridRecommender(BaseUserItemCBFRecommender, _BaseLightFMWrapper):
    """LightFMUserHybridRecommender"""

    RECOMMENDER_NAME = "LightFMUserItemHybridRecommender"

    def __init__(self, URM_train, UCM_train, ICM_train, verbose = True):
        super(LightFMUserItemHybridRecommender, self).__init__(URM_train, UCM_train, ICM_train, verbose = verbose)
        
        # Need to hstack user_features to ensure each UserIDs are present in the model
        eye_users = sps.eye(self.n_users, self.n_users).tocsr()
        self.UCM_train = sps.hstack((eye_users, self.UCM_train)).tocsr()
        eye_items = sps.eye(self.n_items, self.n_items).tocsr()
        self.ICM_train = sps.hstack((eye_items, self.ICM_train)).tocsr()