...
from RecSysFramework.Recommenders.Similarity.Compute_Similarity_Python import Incremental_Similarity_Builder
from RecSysFramework.Recommenders.BaseMatrixFactorizationRecommender import BaseSVDRecommender
    ...
    while start_item < n_items:
        ...
        for col_index_in_block in range(this_block_weight.shape[0]):
            ...
            similarity_builder.add_data_lists(row_list_to_add=relevant_items_partition,
                                              col_list_to_add=np.ones(len(relevant_items_partition), dtype = np.int32) * item_original_index,
