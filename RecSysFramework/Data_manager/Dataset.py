...
from RecSysFramework.Data_manager.DataReader_utils import reconcile_mapper_with_removed_tokens, remove_features
from RecSysFramework.Data_manager.data_consistency_check import assert_URM_ICM_mapper_consistency
from RecSysFramework.Data_manager.DataReader_utils import compute_density
...
class Dataset(object):
    ...
    def verify_data_consistency(self):
        ...
        for URM_name, URM_object in self.AVAILABLE_URM.items():
                ...
                assert not np.isnan(URM_object.data).any(), print_preamble + "Contains NANs in {}".format(URM_name)
    ...
    def _remove_items_and_users(self, items_to_remove = None, users_to_remove = None):
        ...
        if items_to_remove is not None:
            ...
            for URM_name, URM_obj in self.AVAILABLE_URM.items():
                self.AVAILABLE_URM[URM_name] = URM_obj.tocsc()[:, items_to_keep_mask].tocsr()
