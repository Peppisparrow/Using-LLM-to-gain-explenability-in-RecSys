...
from RecSysFramework.Data_manager.DataSplitter import DataSplitter as _DataSplitter
from RecSysFramework.Data_manager.DataReader import DataReader as _DataReader
from RecSysFramework.Data_manager.split_functions.split_train_validation_random_holdout import split_train_in_two_percentage_user_wise, split_train_in_two_percentage_global_sample
from RecSysFramework.Data_manager.DataReader_utils import compute_density, reconcile_mapper_with_removed_tokens
from RecSysFramework.Data_manager.data_consistency_check import assert_disjoint_matrices, assert_URM_ICM_mapper_consistency
        ...
        if not self.allow_cold_users:
            ...
            if user_to_remove.sum() >0 :
                ...
                self.SPLIT_GLOBAL_MAPPER_DICT["user_original_ID_to_index"] = reconcile_mapper_with_removed_tokens(self.SPLIT_GLOBAL_MAPPER_DICT["user_original_ID_to_index"],
                                                                                                                  np.arange(0, len(user_to_remove), dtype=np.int32)[user_to_remove])
