...
from RecSysFramework.Data_manager.IncrementalSparseMatrix import IncrementalSparseMatrix, IncrementalSparseMatrix_FilterIDs
import pandas as pd
...
def reconcile_mapper_with_removed_tokens(key_to_value_dict, values_to_remove):
    ...
    mapper_values_array = np.ones(max(key_to_value_dict.values())+1, dtype=np.int32) * -np.inf
...
def add_boolean_matrix_iterator(original_data_dict):
    ...
    for matrix_name, matrix_object in original_data_dict.items():
        ...
        if np.max(matrix_object.data) != 1.0 or np.min(matrix_object.data) != 1.0:
            ...
            matrix_object_implicit.astype(bool, copy=True)
